...
export class Board {
  ...
  //     if (popped[0] === end[0] && popped[1] === end[1]) {
  //       return true;
  //     }
  ...
  pathExists(start, end, symbol) {
    let visited = this.board.map((row) => row.map((el) => false));
    visited[start[0]][start[1]] = true;
    return this.pathExistsRec(start, end, symbol, visited);
  }
  pathExistsRec(start, end, symbol, visited) {
    if (start[0] === end[0] && start[1] === end[1]) {
      return true;
    }
    let neighbors = this.getNeighbors(start, symbol);
    for (let neighbor of neighbors) {
      if (!visited[neighbor[0]][neighbor[1]]) {
        visited[neighbor[0]][neighbor[1]] = true;
        if (this.pathExistsRec(neighbor, end, symbol, visited)) {
          return true;
        }
      }
    }
